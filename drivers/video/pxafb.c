...
#ifdef CONFIG_PXA3xx
	if (enabled) { /* On-the-fly move/resize doesn't seem to work */
		overlay1fb_disable(ofb);
		enabled = 0;
	}
#endif
	fdadr1 = ofb->fbi->fdadr[DMA_OV1] | (enabled ? 0x1 : 0);
...
static void overlay2fb_setup(struct pxafb_layer *ofb)
{
	...
	if (pfor == OVERLAY_FORMAT_RGB || pfor == OVERLAY_FORMAT_YUV444_PACKED) {
		...
		ofb->fb.var.red.offset = 0;
		ofb->fb.var.red.length = size;
		ofb->fb.var.green.offset = size;
		ofb->fb.var.green.length = (size / div);
		ofb->fb.var.blue.offset = size + (size / div);
		ofb->fb.var.blue.length = (size / div);
		setup_frame_dma(ofb->fbi, DMA_OV2_Y,  -1, start[0], size);
...
static void overlay2fb_disable(struct pxafb_layer *ofb)
{
	uint32_t lccr5 = lcd_readl(ofb->fbi, LCCR5);
	uint32_t toBranch;
	int currentPfor = OVL2C2_TO_PFOR(lcd_readl(ofb->fbi, OVL2C2));

	lcd_writel(ofb->fbi, OVL2C1, ofb->control[0] & ~OVLxC1_OEN);

	toBranch = LCSR1_BS(2);
	if (currentPfor != OVERLAY_FORMAT_RGB
			&& currentPfor != OVERLAY_FORMAT_YUV444_PACKED)
		toBranch |= LCSR1_BS(3) | LCSR1_BS(4);

	lcd_writel(ofb->fbi, LCSR1, toBranch);    /* Clear int statuses */
	ofb->branch_waiting = toBranch;	  /* Mark what ints to wait for */
	lcd_writel(ofb->fbi, LCCR5, lccr5 & ~toBranch);  /* Enable ints */

	/* Signal /running/ DMA channels to branch on next frame */
			       ...
			       struct fb_info *info)
{
	...
	/* - This appears to be not necessary - 
	 * if ((xpos * bpp) % 32)
	 *	return -EINVAL;
	 */
			      ...
			      struct pxafb_info *fbi)
{
	...
	fbi->reg_lccr4 |= (fbi->lccr4 & LCCR4_PAL_FOR_MASK) | (fbi->lccr4 & LCCR4_PCDDIV);
	local_irq_restore(flags);
...
static irqreturn_t pxafb_handle_irq(int irq, void *dev_id)
{
...
#ifdef CONFIG_FB_PXA_OVERLAY
	{
		...
		/* If we are still waiting for any OV2 branches */
		if (fbi->overlay[1].branch_waiting) {
			/* mark any that have branched. */
			fbi->overlay[1].branch_waiting &= ~lcsr1;

			/* If all have now branched, signal complete */
			if (!fbi->overlay[1].branch_waiting)
				complete(&fbi->overlay[1].branch_done);
		}
/* incoming from merges
				   ...
				   struct pxafb_mach_info *inf)
{
	...
	fbi->lccr4 |= (inf->lccr4 | LCCR4_PCDDIV);
